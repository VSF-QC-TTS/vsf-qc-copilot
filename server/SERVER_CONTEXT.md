...
Date: 2026-06-12
Repo area: `server/`
Last full-suite pass: 226 tests, 0 failures (2026-06-11). Latest focused promptfoo/job suite pass: 16 tests, 0 failures (2026-06-12).
...
- Evaluation runs and jobs:
  ...
  - CLI output parser expects `results.outputs[]` and maps rows into `PromptfooResult`.
  ...
  - Promptfoo local runner metadata exists under `tooling/promptfoo-runner` for `promptfoo@0.121.15`; `package-lock.json` still needs to be generated by running npm install when registry access is allowed.
...
- `tooling/promptfoo-runner/package-lock.json` is still missing until npm registry access is allowed.
